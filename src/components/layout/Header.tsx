...
export default function Header({ locationStatus, pharmacyCount, cityName }: HeaderProps) {
    return (
        <header className="h-12 bg-dark-950 border-b border-dark-700/50 flex items-center px-4 shrink-0">
            <div className="flex items-center justify-between w-full max-w-[1920px] mx-auto">
                ...
                <div className="lg:hidden flex items-center">
                    ...
                    {locationStatus === "granted" && cityName && (
                        <div className="inline-flex items-center gap-1.5 text-dark-200 text-xs">
                            <span className="w-1.5 h-1.5 bg-green-500 rounded-full shrink-0" />
                            <span className="truncate max-w-[140px]">{cityName}</span>
                            {pharmacyCount > 0 && (
